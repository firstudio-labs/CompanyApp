{{-- filepath: resources/views/admin/portfolio/edit.blade.php --}}
...
<div class="container-xxl flex-grow-1 container-p-y">
    <h4 class="fw-bold py-3 mb-4">
        <span class="text-muted fw-light">Portfolio /</span> Edit Portfolio
    </h4>
    <div class="row">
        <div class="col-lg-8">
            <div class="card mb-4">
                <div class="card-body">
                    <form id="portfolioForm" action="{{ route('admin.portfolio.update', $portfolio->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <div id="tech-stack-group">
                                @php
                                    $oldTechStacks = old('tech_stack_ids', $portfolio->techStacks->pluck('id')->toArray());
                                @endphp
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('link') is-invalid @enderror" id="link" name="link" value="{{ old('link', $portfolio->link) }}">
                        ...
                        <div class="mb-3">
                            ...
                            @if($portfolio->image_url)
                                <div class="mt-2">
                                    <img id="current-image" src="{{ asset('images/portfolios/' . $portfolio->image_url) }}" alt="{{ $portfolio->title }}" style="width:100px; height:100px; object-fit:cover;">
                                </div>
                            @endif
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <div class="text-center mb-3">
                        <img id="preview-image" src="{{ $portfolio->image_url ? asset('images/portfolios/' . $portfolio->image_url) : asset('assets/img/placeholder-image.png') }}" alt="Preview Gambar" style="width:150px; height:150px; object-fit:cover; border-radius:8px;">
                    </div>
                    <h5 id="preview-title">{{ old('title', $portfolio->title) ?: 'Judul Portfolio' }}</h5>
                    <p><strong>Service:</strong> <span id="preview-service">
                        {{ $services->find(old('service_id', $portfolio->service_id))?->title ?? '-' }}
                    </span></p>
                    <p><strong>Tech Stack:</strong> <span id="preview-techstack">
                        @php
                            $selected = old('tech_stack_ids', $portfolio->techStacks->pluck('id')->toArray());
                            $names = $techStacks->whereIn('id', $selected)->pluck('name')->toArray();
                        @endphp
                        {{ $names ? implode(', ', $names) : '-' }}
                    </span></p>
                    <p><strong>Deskripsi:</strong> <span id="preview-description">
                        {{ \Illuminate\Support\Str::limit(strip_tags(old('description', $portfolio->description)), 100) ?: '-' }}
                    </span></p>
                    <p><strong>Link:</strong> <span id="preview-link">{{ old('link', $portfolio->link) ?: '-' }}</span></p>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    document.getElementById('image_url').addEventListener('change', function(e) {
        ...
        if (file) {
            ...
            preview.src = "{{ $portfolio->image_url ? asset('images/portfolios/' . $portfolio->image_url) : asset('assets/img/placeholder-image.png') }}";
        }
    });
