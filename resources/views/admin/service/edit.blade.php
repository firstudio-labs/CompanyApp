                    ...
                    <form id="serviceForm" action="{{ route('admin.service.update', $service->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            @if($service->image_banner)
                                <img src="{{ asset($service->image_banner) }}" alt="Banner Saat Ini" class="mt-2" style="max-height: 120px;">
                            @endif
                        </div>
                        <div class="mb-3">
                            ...
                            @if($service->image_url)
                                <img src="{{ asset($service->image_url) }}" alt="Gambar Saat Ini" class="mt-2" style="max-height: 120px;">
                            @endif
                        </div>
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <div class="text-center mb-3">
                        <img id="preview-image" src="{{ $service->image_url ? asset($service->image_url) : asset('assets/img/placeholder-image.png') }}" alt="Preview Gambar" style="width:150px; height:150px; object-fit:cover; border-radius:8px;">
                    </div>
                    <h5 id="preview-title">{{ old('title', $service->title) ?: 'Judul Layanan' }}</h5>
                    <div id="preview-description" class="mb-2 text-muted">{!! old('description', $service->description) ?: 'Deskripsi akan tampil di sini...' !!}</div>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    document.getElementById('image_url').addEventListener('change', function(e) {
        ...
        if (file) {
            ...
            preview.src = "{{ $service->image_url ? asset($service->image_url) : asset('assets/img/placeholder-image.png') }}";
        }
    });
</script>
@endsection
